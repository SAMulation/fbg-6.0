            ...
            Ok(message) => {
                ...
                if let Ok(text) = message.to_str() {
                    if text.starts_with("/message") {
                        ...
                        let chat_message = text.to_string(); // Example: Extract relevant data from the message
                        lobby.lock().await.broadcast_message(chat_message);
                        ...
                        for (id, client) in clients.lock().await.iter() {
                            ...
                            if client.send(text.to_string()).is_err() {
